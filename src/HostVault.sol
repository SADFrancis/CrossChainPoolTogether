...
/// @title  Host Vault
/// @author Sean Francis
/// @notice This is a spinoff of PoolTogether's Prize Vault contract
/// But the kicker here is that this is used to convert Tips into lottery tickets
/// The Host of the contract is the organization/entity/person receiving tips
/// Depositors deposit money (tips) into this contract, to receieve delegation for a chance to win the pool prize
/// but they forfeit their deposits to the host
contract HostVault is TwabERC20, Claimable, IERC4626, ILiquidationSource, Ownable {
    ...
    /// @notice Address of the Host Address which doesn't necessarily have to be the Owner of the contract
    ...
    /// @notice Thrown when caller of function is not the host of the contract
    error CallerNotHost(address caller, address host);
    ...
    /// @notice Require the caller to be the host of the contract
    modifier onlyHost(){
        if(msg.sender != s_host){
            revert CallerNotHost(msg.sender, s_host);
        }
        _;
    }
    ...
    /// TODO - Need to disable with onlyHost
    ...
    // TODO - ensure the balance of asset deposits go to the Host and not the depositor
    ...
    // TODO - disable and prevent non Hosts from withdrawing
    ...
    /// TODO - disable and prevent non Hosts from redeeming
    ...
    /// @notice Change Host Address
    /// @param _newHost Address of the new Host
